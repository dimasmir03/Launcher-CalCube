...
namespace WindowsFormsApp3
{
    public partial class Form1 : Form
    {
        ...
        public string path = "D:\\Work\\KalCube\\testpath\\";
        public string exename = "ver.txt";//"CalCube.exe";
        public string url = "http://web2936.craft-host.ru/";
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                RegistryKey key;
                key = Registry.CurrentUser.CreateSubKey("CalCube");
            ...
            {
                version = (string)Registry.CurrentUser.OpenSubKey("CalCube").GetValue("ver");
            }


            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url + "ver.txt");
                ...
                wc.DownloadProgressChanged += (s, g) => { 
                    if (g.ProgressPercentage == 100)
                    {
                        if (!File.Exists(Path.GetFullPath(path + exename)))
                        {
                            ...
                            RegistryKey key;
                            key = Registry.CurrentUser.CreateSubKey("CulCube");
